...
X_mean = np.load("logistic_X_mean.npy")
X_std = np.load("logistic_X_std.npy")
with open(x_test_path) as ft:
	features = np.array(ft.readline().strip("\n").split(","))
	X_test =  np.array([line.strip("\n").split(",")[1:] for line in ft], dtype=float)
X_test , _, _ = tool_normalize(X_test, False, X_mean, X_std)
Y_test_predict = tool_predict(X_test, w, b)
with open(output_path.format("logistic"), "w") as fout:
	fout.write("id,label\n")
